...
4. **No max-height cap on a single tall/portrait image.** Considered adding
   ...
   verify, and neither the brief nor spec §12 asks for it — left out
...
## Fix round 1 (review: spec ✅, quality approved with 1 Important, 2 Minor)

### Important: the version-skew white screen

The reviewer's fixture-bug finding above was correct as far as it went, but
it revealed a second, more serious problem it didn't itself fix: `PostCard`
read `post.media.length` **unconditionally**. In a test that surfaced as a
fixture error with an easy fix. In production it is a deploy-window bug:

- `scripts/deploy.sh` copies the new web bundle into nginx's serving
  directory **before** it reloads the api process, then polls health for up
  to 60 seconds.
- `media` is introduced on both sides in this branch; `main`'s
  `post-views.ts` has no such field.
- For that window, the new bundle (already reading `post.media`) is live
  and talking to the still-running **old** api, which answers without
  `media`.
- `apiClient.ts` does no runtime shape validation (`res.json() as T`), and
  the app has no error boundary anywhere. A throw inside a React render
  unmounts the tree.

So the failure mode during that window is not "a post renders without its
photos" — it's a **blank `/beranda` and blank profile pages for every
visitor**, plus the same exposure through `PostFeed`'s `prepend`/`replace`
for a post created mid-window.

**Fix:** guarded the read. In `apps/web/src/user/PostCard.tsx`:

```ts
const media = post.media ?? [];
```

used in place of every bare `post.media` reference (the `.length > 0`
check, the `data-count` attribute, and the `.map`). The type (`PostView.media:
MediaView[]`, required, never absent) is left exactly as it was — it's
honest about what a *healthy* api returns. The guard is for the minute the
api answering isn't the current one, which is a real, deploy-ordering-
guaranteed window, not a hypothetical.

**Test added** (`PostCard.test.tsx`, in the same `describe` block):
`"renders without throwing when \`media\` is missing from the response
entirely (version-skew deploy window)"`. Builds a `PostView`-shaped object
with every field except `media`, cast through `as unknown as PostView`
(deliberately lying about the type, the same way the real skewed response
does), renders it, and asserts no `.post-card-media` block and no `<img>` —
i.e., the card degrades to "no images" rather than throwing.

**Mutation evidence.** Changed `const media = post.media ?? [];` to
`const media = post.media;` and reran `bun test src/user/PostCard.test.tsx`:

```
TypeError: undefined is not an object (evaluating 'media.length')
      at <anonymous> (.../@testing-library/react/dist/act-compat.js:74:13)
      at renderRoot (.../@testing-library/react/dist/pure.js:189:18)
      at <anonymous> (apps/web/src/user/PostCard.test.tsx:169:27)
(fail) PostCard — the media slot (Task 9, spec §3, §4, §5.1, §12) > renders without throwing when `media` is missing from the response entirely (version-skew deploy window) [9.45ms]
...
Exactly the new test reddened, for its own reason (the thrown
`TypeError`), with none of the other 22 tests in the file affected. No DOM
node appeared in the failure output — the throw happens inside React's
render, before `renderCard` even returns a `container` to touch. Restored
the guard (`const media = post.media ?? [];`) immediately after.

### Minor: spec citation drift

Comments and the describe block's name cited "§7" (the composer) for
things §7 does not govern. Corrected in `PostCard.tsx` and
`PostCard.test.tsx`:

- The media-slot comment (thumbnails, `data-count`) now cites **§5.1**
  (delivery proxies, thumbnails named as Phase 4's job) and **§3** (the
  media slot itself, in "what the parent specs already settle").
- The `width`/`height` comment already correctly cited **§4** (the model —
  where those columns are defined) — left unchanged.
- The `alt=""` comment already correctly cited **§12** (honest
  limitations — no alt text) — left unchanged, expanded slightly to name
  the subsection.
- The test file's `describe("PostCard — the media slot …")` name changed
  from `(Task 9, spec §7/§12)` to `(Task 9, spec §3, §4, §5.1, §12)`,
  naming every section the block actually exercises.
- This report's own §7 mis-citation (in "Judgement calls," item 4) was
  also corrected to §12 — the only wrong one still asserting a claim about
  spec content; the two remaining §7 mentions above are a literal quote of
  a docstring and a description of what was corrected, not live citations,
  and are left as an accurate record of what happened.

Per the reviewer's instruction, no 2- or 4-image test was added (deferred;
CSS-only difference at those counts).

### Verification

```
bun test        -> 745 pass, 0 fail, 1728 expect() calls, 45 files, ~18s
bun run typecheck -> clean, no output
```

`git status` is clean after committing (this report's own edits are the
last thing committed; `.superpowers/` is gitignored, so this file will not
appear in `git status` — expected and unrelated to tree cleanliness).

**Out of scope, as instructed:** reordering `scripts/deploy.sh` to reload
the api before swapping the bundle, which would close this window at its
source, was not touched — recorded by the coordinator for the final
