...
export class DecideJoinRequest {
  ...
  async execute(input: {
    ...
    // The graceful pre-check, beside the tier-active check above and OUTSIDE
    // the transaction — the same division of labour `markPaid` uses for its
    // own "already active" case. Cheap, and it answers the ordinary
    // (non-racing) case with a clear 409 before any write is attempted. It is
    // NOT the guarantee — see the try/catch inside the transaction below for
    // that — because this is a plain read and two decisions racing each other
    // could both pass it. Scoped to `"approved"` only: rejecting never creates
    // a subscription, so a member already active in this tier is not this
    // decision's business.
