 ...
 * is deliberately not behind `requireAuth`. It is authenticated instead by a
 * static header secret, `X-CALLBACK-TOKEN`, verified in CONSTANT TIME before
 * anything else happens.
 *
 * Retire-telegram Task 2 removed the sibling `POST /telegram` that used to be
 * mounted here and recorded a joining member's Telegram user id. The rule it
 * shared with `/xendit` still holds for anything added later: do not mount
 * anything under `/webhooks` that is not token-verified, and do not "fix" this
 * route by adding `requireAuth` — that would silently stop every real payment
 * from being credited.
 */
