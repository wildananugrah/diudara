...
/**
 * The community-side rows a webhook delivery references, INSERTED DIRECTLY.
 *
 * These two helpers used to drive the real HTTP surface — `POST /communities`,
 * `POST /communities/:id/tiers` and `POST /c/:slug/checkout` — but
 * retire-telegram Task 4 deleted all three with the community-scoped API.
 * `HandlePaymentWebhook`'s community branch is NOT deleted (it is Task 5's seam),
 * so every test below still covers live code, and dropping them for want of a
 * setup route would have taken the branch's only coverage with them.
 *
 * The rows are exactly what `StartCheckout` used to write: a `pending`
 * subscription, a `pending` transaction whose amount is the tier's price and whose
 * `payment_method` is the literal `"invoice"` placeholder, and — the anchor the
 * replay defence rests on — a `gateway_reference_id` recorded BEFORE any webhook
 * arrives. Nothing here is read back by the assertions as a stand-in for
 * production behaviour: the one test that asserted on `StartCheckout`'s own write
 * ("records the provider's invoice id on the transaction at CHECKOUT") went with
 * that use case, because against this helper it would only have restated its own
 * setup.
 *
 * Ids are generated by the database, so `externalId` is a real transaction id and
 * `invoiceId` a value the handler must match `body.id` against, exactly as before.
 */
let seedCounter = 0;

async function checkout(
  _a: ReturnType<typeof app>,
  tier: { priceAmount?: number; billingCycle?: string } = {}
) {
  seedCounter += 1;
  const [creator] = await db.insert(creators).values({ name: "Budi" }).returning();
  const [community] = await db
    .insert(communities)
    .values({
      creatorId: creator.id,
      name: "Kelas Budi",
      slug: `kelas-budi-${seedCounter}-${Date.now()}`,
    })
    .returning();
  ...
  return {
    ...
    slug: community.slug,
    tierId: created.id,
    ...(await buy(_a, community.slug, created.id)),
  };
}

/**
 * One purchase of `tierId` by the SAME payer every time (+6281234567890) — so
 * calling it twice against one tier is exactly the double-submit that produces two
 * pending subscriptions for one (member, tier).
 *
 * `slug` is accepted and unused: it identified the community for the deleted
 * checkout route, and the call sites read better naming the community they are
 * buying into. The member is found-or-created on the number, the way
 * `MemberRepositoryPort.findOrCreateByWhatsappNumber` did.
 */
async function buy(_a: ReturnType<typeof app>, _slug: string, tierId: string) {
  const [existingMember] = await db
    .select()
    .from(members)
    .where(eq(members.whatsappNumber, "+6281234567890"));
  const member =
    existingMember ??
    (
      await db
        .insert(members)
        .values({ whatsappNumber: "+6281234567890", name: "Siti" })
        .returning()
    )[0];
  ...
  const [subscription] = await db
    ...
    .values({ memberId: member.id, tierId, status: "pending" })
    .returning();
  seedCounter += 1;
  const [transaction] = await db
    ...
    .values({
      ...
      // The idempotency ANCHOR, written at checkout time exactly as
      // `StartCheckout` wrote it: without it the handler has nothing of ours to
      // check `body.id` against.
      gatewayReferenceId: `inv_seed_${seedCounter}`,
    })
    .returning();
  ...
  return {
    ...
    invoiceId: transaction.gatewayReferenceId!,
  };
}
