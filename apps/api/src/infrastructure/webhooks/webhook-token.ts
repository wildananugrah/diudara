 ...
 * reason: two inbound webhooks now depend on it, and both authenticate the same
 * way — with a STATIC token in a header rather than an HMAC over the payload.
 *
 *   `X-CALLBACK-TOKEN`                     — Xendit  (routes/webhooks.ts)
 *   `X-Telegram-Bot-Api-Secret-Token`      — Telegram `setWebhook` secret_token
 *
 * A static token authenticates the SENDER, not the message, so this comparison is
 * the only thing standing between an attacker and a forged event — a forged
 * payment for Xendit, a forged `chat_member` join for Telegram. A plain `===`
 * leaks the token byte-by-byte under timing analysis.
 *
