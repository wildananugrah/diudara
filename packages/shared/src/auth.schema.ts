...
/**
 * `POST /users/signup` — a person's account, and the only signup this codebase
 * has. A creator-only `signupSchema`/`loginSchema` pair stood above this one,
 * sharing two module-level `email`/`password` field builders; retire-telegram
 * Task 7's fix round deleted them with `POST /auth/signup` and
 * `POST /auth/login`, the routes that were their only consumers. Nothing here
 * changed as a result: these schemas always declared their own fields inline
 * rather than reusing those builders.
 *
 * `handle` allows up to 31 characters here (not the domain's 30) so a leading
 * `@` a caller pasted in survives validation; `normalizeHandle` strips it
 * before the domain's 3-30 rule (`isValidHandle`) is enforced on the
 * normalised value. The WhatsApp regex is copied FIELD FOR FIELD from the
 * checkout form's (tolerant of a leading 0 or +62) so the two never disagree
 ...
 * deliberately NOT `.toLowerCase()` — case-folding happens inside
 * `RequestPasswordReset` via `normalizeEmail`, the same split
 * `userSignupSchema` above already uses for the same field. (The deleted
 * creator `signupSchema` case-folded in the schema instead; this sentence
 * used to contrast the two.)
 */
